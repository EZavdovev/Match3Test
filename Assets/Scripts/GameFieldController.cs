    ...
    private void InitField()
    {
        ...
        for (int i = 0; i < countHeightBubble; i++)
        {
            for (int j = 0; j < countWidthBubble; j++)
            {
                ...
                bubble.transform.localPosition = new Vector3(xPos, yPos, 0f);
            ...
            for(int j = downIndexForCheck; j < countHeightBubble; j++)
            {
                ...
                if (fieldBubbles[j, i] != null)
                {
                    ...
                    if (isFirstEmptyInArray)
                    {
                        ...
                        fieldBubbles[j - yPosOffset, i - XPosOffset].ChangePos(countWidthBubble,countHeightBubble, i - XPosOffset, j - yPosOffset);
                    ...
                    else
                    {
                        if (XPosOffset > 0)
                        {
                            ...
                            fieldBubbles[j, i - XPosOffset].ChangePos(countWidthBubble, countHeightBubble, i - XPosOffset, j);
                            fieldBubbles[j, i] = null;
                        }
                    }
                }
